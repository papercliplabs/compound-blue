  ...
  const formSchema = useMemo(() => {
    return z
      .object({
        ...
        maxSlippageTolerance: z.coerce.number().min(0.1).max(MAX_SLIPPAGE_TOLERANCE_PCT),
      })
      .refine(
        (data) => {
  ...
  const onSubmit = useCallback(
    async ({
      ...
      const action = await prepareMarketLeveragedBorrowAction({
        ...
        requiresPublicReallocation: false, // TODO: set this correctly
                ...
                render={({ field: { value, onChange } }) => (
                  <FormItem className="flex flex-col gap-2">
                    <div className="flex items-center justify-between">
                      <FormLabel className="flex items-center gap-1 text-content-secondary">
                        Max Slippage
                        <TooltipPopover>
                          <TooltipPopoverTrigger>
                            <Info size={16} />
        ...
        >
          <ActionFlowSummary>
            <ActionFlowSummaryAssetItem
              asset={market.collateralAsset}
              actionName="Add"
              descaledAmount={initialCollateralAmount}
              amountUsd={initialCollateralAmount * (market.collateralAsset.priceUsd ?? 0)}
            />
          </ActionFlowSummary>
          <ActionFlowReview className="flex flex-col gap-4">
            <MetricChange
              name={`Collateral (${market.collateralAsset.symbol})`}
              initialValue={formatNumber((collateralTokenBalance ?? 0) * (market.collateralAsset.priceUsd ?? 0), {
                currency: "USD",
              })}
              finalValue={formatNumber(
                (userPosition?.collateralAssetsUsd ?? 0) +
                  descaleBigIntToNumber(preparedAction.collateralAssetChange, market.collateralAsset.decimals) *
            ...
            <MetricChange
              ...
              initialValue={formatNumber(userPosition?.borrowAssetsUsd ?? 0, {
                currency: "USD",
              })}
              finalValue={formatNumber(
                (userPosition?.borrowAssetsUsd ?? 0) +
                  descaleBigIntToNumber(preparedAction.loanAssetChange, market.loanAsset.decimals) *
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1 label-md">
                <span className="text-content-secondary">
                  (
                  {formatNumber(userPosition?.ltv ?? 0, {
                    style: "percent",
                  })}
                </span>
                <ArrowRight size={14} className="stroke-content-secondary" />
                {formatNumber(preparedAction.newLtv, {
                  style: "percent",
                })}
